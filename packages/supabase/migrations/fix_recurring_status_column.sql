-- Migration: Fix Recurring Booking Status Column Default
-- The original definition `DEFAULT 'active'` causes type mismatch errors (text vs enum).
-- We must alter the column to use an explicit cast for the default value.

ALTER TABLE public.recurring_bookings
ALTER COLUMN status
SET DEFAULT 'active'::public.recurring_booking_status;

-- Double check RPC to ensure it accepts the fix
...
BEGIN
    ...
    INSERT INTO public.recurring_bookings (
        ...
        'active'::public.recurring_booking_status
