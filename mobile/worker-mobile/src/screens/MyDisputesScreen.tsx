/**
 * My Disputes Screen (Worker)
 * Shows list of worker's disputes with status badges
 */
...
const ISSUE_ICONS: Record<string, string> = {
    service_quality: '⭐', no_show: '🚫', payment: '💳', damage: '🔨', other: '📝',
};
...
export function MyDisputesScreen({ navigation }: Props): React.JSX.Element {
    ...
    const fetchDisputes = useCallback(async () => {
        ...
        try {
            const { data, error } = await supabase
                .from('disputes')
                .select('*')
                .or(`worker_id.eq.${user.id},raised_by.eq.${user.id}`)
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('worker-disputes-updates')
    ...
    const renderDispute = ({ item }: { item: Dispute }) => {
        ...
        return (
            <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('DisputeDetail', { disputeId: item.id })} activeOpacity={0.7}>
                <View style={styles.cardHeader}>
                    <View style={styles.cardLeft}>
                        <Text style={styles.issueIcon}>{ISSUE_ICONS[item.issue_type] || '📝'}</Text>
                        <Text style={styles.cardTitle}>{item.issue_type.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</Text>
                    </View>
