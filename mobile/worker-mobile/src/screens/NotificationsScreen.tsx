...
export function NotificationsScreen(): React.JSX.Element {
    ...
    const getIcon = (type: string | null) => {
        switch (type) {
            case 'new_booking_available': return '📋';
            case 'new_assignment': return '🎯';
            case 'job_reassigned': return '🔄';
                ...
                <View style={styles.notifContent}>
                    ...
                    {item.metadata?.amount > 0 && (
                        <Text style={styles.notifDetail}>
                            💰 ₹{item.metadata.amount} • ⏱ {item.metadata.duration_minutes || 60} min
                        </Text>
                    )}
                    {item.metadata?.address && (
                        <Text style={styles.notifDetail} numberOfLines={1}>
                            📍 {item.metadata.address}{item.metadata.city ? `, ${item.metadata.city}` : ''}
                        </Text>
                    )}
                ...
                ListEmptyComponent={
                    !loading ? (
                        <View style={styles.emptyContainer}>
                            ...
                            <Text style={styles.emptySubtext}>You'll see booking alerts here</Text>
                        </View>
                    ) : null
                }
            />
...
const styles = StyleSheet.create({
    ...
    unreadBanner: { backgroundColor: '#064e3b', marginHorizontal: 16, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, marginBottom: 8 },
    unreadBannerText: { color: '#6ee7b7', fontSize: 12, fontWeight: '600' },
    listContent: { paddingHorizontal: 16, paddingBottom: 32 },
    notifCard: {
        backgroundColor: '#1e293b', borderRadius: 12, padding: 14, marginBottom: 8,
        borderWidth: 1, borderColor: '#334155',
    },
    notifCardUnread: { borderColor: '#10b981', borderLeftWidth: 3 },
    ...
    notifDetail: { fontSize: 12, color: '#64748b', marginBottom: 2 },
