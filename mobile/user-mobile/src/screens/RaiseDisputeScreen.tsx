 ...
 * User can raise a dispute against a booking
 */
...
export function RaiseDisputeScreen({ route, navigation }: Props): React.JSX.Element {
    const { bookingId, serviceName, workerId } = route.params;
    const { user } = useUser();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert(
                ...
                [{
                    text: 'View Details',
                    onPress: () => navigation.replace('DisputeDetail', { disputeId: data.id }),
                }]
            );
        } catch (err: any) {
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.issueGrid}>
                    {ISSUE_TYPES.map(type => (
                        <TouchableOpacity
                            key={type.key}
                            style={[styles.issueChip, issueType === type.key && styles.issueChipSelected]}
                            onPress={() => setIssueType(type.key)}
                            activeOpacity={0.7}
                        >
                            <Text style={styles.issueChipIcon}>{type.icon}</Text>
                            <Text style={[styles.issueChipText, issueType === type.key && styles.issueChipTextSelected]}>
                                {type.label}
                            </Text>
                        </TouchableOpacity>
                    ))}
...
const styles = StyleSheet.create({
    ...
    issueChipSelected: { backgroundColor: '#312e81', borderColor: '#6366f1' },
    issueChipIcon: { fontSize: 18, marginRight: 8 },
    issueChipText: { fontSize: 14, color: '#94a3b8', fontWeight: '500' },
    issueChipTextSelected: { color: '#a5b4fc' },
    textArea: {
        backgroundColor: '#1e293b', borderWidth: 1, borderColor: '#334155',
        borderRadius: 12, padding: 16, fontSize: 15, color: '#f8fafc',
    ...
    removeBtn: {
        position: 'absolute', top: 2, right: 2, backgroundColor: 'rgba(0,0,0,0.7)',
    ...
    addImageBtn: {
        width: 80, height: 80, borderRadius: 10, borderWidth: 1.5, borderColor: '#334155',
        borderStyle: 'dashed', justifyContent: 'center', alignItems: 'center', backgroundColor: '#1e293b',
    },
